...
const MetricCards = ({ metrics, loading }: MetricCardsProps) => {
  if (loading) {
    return (
  ...
  const getColorClasses = (color: string) => {
    switch (color) {
      ...
      case 'blue': return 'text-blue-400 border-blue-400/20';
      case 'orange': return 'text-orange-400 border-orange-400/20';
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      {metrics.map((metric, index) => (
        <div 
          key={index}
          className={`glass-card rounded-xl p-4 border ${getColorClasses(metric.color)} hover:scale-105 transition-transform duration-200`}
        >
