...
import { Lock, Shield, Trophy, Coins } from 'lucide-react';
    ...
    <div className={`transition-all duration-1000 delay-500 ${lockerPhase >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
      ...
      <div className="relative bg-black/40 backdrop-blur-xl border-2 border-violet-500/30 rounded-xl p-8 overflow-hidden group hover:scale-105 hover:border-violet-500/60 transition-all duration-500 mb-12">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2">
          ...
          <span className="text-violet-400 font-mono text-xs">VAULT ACTIVE</span>
        </div>
        ...
        <div className="relative z-10">
          <div className="text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="bg-black/30 backdrop-blur-sm border border-violet-500/30 rounded-xl p-6 text-center hover:border-violet-500/50 transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center">
                    <Lock className="w-5 h-5 text-violet-400 mr-2" />
                    <span className="text-violet-300 font-semibold font-mono">ACTIVE_LOCKERS</span>
                  </div>
                </div>
                <div className="text-3xl font-black text-violet-400 font-mono">
                  {loading ? '[SCANNING...]' : `${lockerData.totalLockers.toLocaleString()}`}
                </div>
                <div className="text-xs text-gray-400 font-mono mt-2">[EARNING_REWARDS]</div>
              </div>
              
              <div className="bg-black/30 backdrop-blur-sm border border-violet-500/30 rounded-xl p-6 text-center hover:border-violet-500/50 transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center">
                    <Coins className="w-5 h-5 text-violet-400 mr-2" />
                    <span className="text-violet-300 font-semibold font-mono">PENDING_FEES</span>
                  </div>
                </div>
                <div className="text-3xl font-black text-violet-400 font-mono">
                  {loading ? '[SCANNING...]' : `${lockerData.accumulatedFees.toLocaleString()}`}
                </div>
                <div className="text-xs text-gray-400 font-mono mt-2">[TOKENS_ACCUMULATED]</div>
              </div>
            </div>
