            ...
            {tiers.map((tier, index) => (
              <div
                ...
                className={`relative group cursor-pointer transition-all duration-300 ${
                  hoveredTier === index ? 'scale-105' : ''
                }`}
                onMouseEnter={() => setHoveredTier(index)}
                onMouseLeave={() => setHoveredTier(null)}
              >
                {/* Tier Badge */}
                <div className={`bg-black/40 backdrop-blur border border-${tier.color}/40 rounded-lg p-3 hover:border-${tier.color}/60 transition-all ${
                  ...
                  <div className={`absolute left-1/2 transform -translate-x-1/2 z-20 ${
                    ...
                    <div className="bg-black/90 backdrop-blur-xl border border-cyan-500/30 rounded-lg p-3 min-w-48">
                      ...
                      <div className="flex justify-between text-xs font-mono">
                        ...
                        <span className={`text-${tier.color}`}>{tier.multiplier}</span>
                      </div>
                      <div className="flex justify-between text-xs font-mono">
