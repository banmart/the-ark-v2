...
const LockerTiersSection = ({
  ...
  const TierCard = ({
    tier
      ...
      {tier.special && <div className="absolute top-2 right-2 bg-orange-500 text-black px-3 py-1 rounded-full text-xs font-bold animate-pulse">
          LEGENDARY
        </div>}
      ...
      <div className="relative z-10">
        ...
        <Link to="/locker" className={`block w-full bg-gradient-to-r ${tier.buttonGradient} text-black font-bold py-3 rounded-lg text-center hover:scale-105 transition-transform relative z-10 ${tier.special ? 'shadow-lg shadow-orange-500/30' : ''}`}>
          {tier.special ? 'Become Legendary' : `${tier.name === 'BRONZE' ? 'Enter' : 'Ascend to'} ${tier.name.charAt(0) + tier.name.slice(1).toLowerCase()}`}
        </Link>
      </div>
    </div>;
  return <section className="relative z-30 py-20 px-6 bg-gradient-to-b from-transparent via-cyan-500/5 to-transparent">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16 relative z-10">
          {tiers.map((tier, index) => <TierCard key={tier.name} tier={tier} />)}
        </div>
        ...
        <div className="text-center relative z-10">
          <Link to="/locker" className="inline-flex items-center bg-gradient-to-r from-cyan-500 to-blue-500 text-black font-bold px-12 py-4 rounded-full text-lg hover:scale-105 transition-transform shadow-lg shadow-cyan-500/30 relative z-10">
